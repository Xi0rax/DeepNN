...
class MLPTorchMultiLayerSigmoid(nn.Module):
    def __init__(self, in_size, hidden_sizes, out_size):
        super(MLPTorchMultiLayerSigmoid, self).__init__()

        # Создаем список слоев на основе размеров из hidden_sizes
        layers_list = []
        prev_size = in_size
...
class MLPTorchMultiLayerReLU(nn.Module):
    def __init__(self, in_size, hidden_sizes, out_size):
        super(MLPTorchMultiLayerReLU, self).__init__()
